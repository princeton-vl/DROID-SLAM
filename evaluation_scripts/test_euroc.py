...
def image_stream(datapath, image_size=[320, 512], stereo=False):
    """ image generator """
    ...
    R_l = np.array([0.999966347530033, -0.001422739138722922, 0.008079580483432283, 0.001365741834644127, 0.9999741760894847, 0.007055629199258132, -0.008089410156878961, -0.007044357138835809, 0.9999424675829176]).reshape(3,3)
    ...
    R_r = np.array([0.9999633526194376, -0.003625811871560086, 0.007755443660172947, 0.003680398547259526, 0.9999684752771629, -0.007035845251224894, -0.007729688520722713, 0.007064130529506649, 0.999945173484644]).reshape(3,3)
    ...
    images_left = sorted(glob.glob(os.path.join(datapath, 'mav0/cam0/data/*.png')))[::2]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--keyframe_thresh", type=float, default=3.4)
    ...
    images_list = sorted(glob.glob(os.path.join(args.datapath, 'mav0/cam0/data/*.png')))[::2]
